  ...
  try {
    ...
    if (role === 'parent') {
      const { data: parentRecord, error: parentError } = await supabaseAdmin
        .from('parents')
        .select(
          'student_id, students!parents_student_id_fkey(id, profiles!students_id_fkey(full_name))'
        )
        .eq('id', req.user.id)
        .single();
      ...
      if (parentError || !parentRecord) {
        return res
          .status(404)
          .json({ success: false, error: 'No linked student found for this parent' });
      }
...
router.get('/all', authenticate, requireWarden, async (req, res, next) => {
  try {
    ...
    const summary = {
      ...
      pending_count: payments.filter((p) => p.status === 'pending' || p.status === 'processing')
        .length,
    };
...
router.post('/create-order', authenticate, async (req, res, next) => {
  try {
    ...
    if (payment.status !== 'pending' && payment.status !== 'processing') {
      return res
        .status(400)
        .json({ success: false, error: `Payment is already ${payment.status}` });
    }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      let query = supabaseAdmin.from('students').select('id, profiles!students_id_fkey(full_name)');
      ...
      for (const bill of bills) {
        const { error: insertError } = await supabaseAdmin.from('fee_payments').insert(bill);
        ...
        if (insertError) {
          if (insertError.code === '23505') {
            ...
            logger.error(
              `Failed to insert bill for student ${bill.student_id}: ${insertError.message}`
            );
            skipped++;
          }
        } else {
...
router.post('/send-reminders', authenticate, requireWarden, async (req, res, next) => {
  try {
    ...
    logger.info(
      `Fee reminders sent: ${remindersSent} for ${(pendingPayments || []).length} payments`
    );
