...
// POST / — requireStudent
router.post('/', authenticate, requireStudent, validate(staffFeedbackSchema), async (req, res, next) => {
  try {
    ...
    if (existingReview && existingReview.length > 0) {
      return res.status(400).json({ success: false, error: 'You have already reviewed this staff member today' })
    }
