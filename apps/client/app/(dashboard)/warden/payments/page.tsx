...
export default function WardenPaymentsPage() {
  ...
  useEffect(() => {
    ...
    apiGet('/api/payments/last-reminder').then(res => {
    ...
    <div className="min-h-screen bg-white px-6 py-10 max-w-6xl mx-auto">
      ...
      {tab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 flex-1">
              <div className="border border-gray-100 rounded-xl p-6 hover:border-gray-300 transition-colors">
                ...
                <p className="text-2xl font-medium text-green-600">{fmt(summary?.total_collected ?? 0)}</p>
              </div>
              <div className="border border-gray-100 rounded-xl p-6 hover:border-gray-300 transition-colors">
                <p className="text-xs text-gray-400 mb-1">Total Pending</p>
                <p className="text-2xl font-medium text-red-500">{fmt(summary?.total_pending ?? 0)}</p>
              </div>
              <div className="border border-gray-100 rounded-xl p-6 hover:border-gray-300 transition-colors">
            ...
            <div className="text-right">
              ...
              {lastReminder && (
                <p className="text-xs text-gray-400 mt-1">
                  Last sent: {new Date(lastReminder).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 outline-none focus:border-gray-400">
            ...
            <select value={filterFeeType} onChange={e => setFilterFeeType(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 outline-none focus:border-gray-400">
              <option value="">All Fee Types</option>
              <option value="combined">Hostel + Mess</option>
              <option value="hostel">Hostel</option>
              <option value="mess">Mess</option>
            </select>
            <select value={filterBilling} onChange={e => setFilterBilling(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 outline-none focus:border-gray-400">
            ...
            />
            <button onClick={fetchPayments} className="border border-gray-200 text-gray-600 rounded-lg px-4 py-2.5 text-sm hover:border-gray-400 transition-colors">
              Apply
            </button>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    ...
                    <th className="text-left px-4 py-3 text-xs text-gray-400 font-medium">Fee Plan</th>
                    ...
                    <th className="text-left px-4 py-3 text-xs text-gray-400 font-medium">Method</th>
                ...
                <tbody>
                  {payments.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="text-center py-12 text-gray-400 text-sm">No payments found</td>
                    </tr>
                  ) : payments.map(p => (
                    <tr key={p.id} className={`border-b transition-colors ${p.is_overdue ? 'bg-red-50 border-red-100' : 'border-gray-50 hover:bg-gray-50'}`}>
                      <td className="px-4 py-3">
                        <div className="text-gray-900 font-medium">{p.students?.profiles?.full_name || '—'}</div>
                      ...
                      <td className="px-4 py-3 text-gray-500">{new Date(p.due_date).toLocaleDateString('en-IN')}</td>
                      <td className="px-4 py-3">{statusBadge(p.status)}</td>
                      <td className="px-4 py-3 text-xs text-gray-500 capitalize">{p.payment_method || '—'}</td>
                      <td className="px-4 py-3">
      ...
      {tab === 'generate' && (
        <div className="space-y-6 max-w-xl">
          <div>
            <h2 className="text-sm font-medium text-gray-900 mb-1">Generate Fee Bills for Students</h2>
            <p className="text-xs text-gray-400">Creates a pending payment record for every active student.</p>
          </div>

          <div className="space-y-4">
            {/* Step 1 */}
            <div className="border border-gray-100 rounded-xl p-5">
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">Step 1 — Select Fee Structure</p>
              <select
            ...
            {/* Step 2 */}
            <div className="border border-gray-100 rounded-xl p-5">
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">Step 2 — Period Label</p>
              <input
                type="text"
                placeholder={selectedStructure?.billing_period === 'monthly' ? 'May-2025' : '2025-26'}
                value={periodLabel}
                onChange={e => setPeriodLabel(e.target.value)}
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-gray-400 w-full"
              />
              <p className="text-xs text-gray-400 mt-1">
                {selectedStructure?.billing_period === 'monthly' ? 'Format: May-2025' : 'Format: 2025-26'}
              </p>
            </div>

            {/* Step 3 */}
            <div className="border border-gray-100 rounded-xl p-5">
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">Step 3 — Due Date</p>
              <input
                type="date"
                value={dueDate}
            ...
            <div className="border border-gray-100 rounded-xl p-5">
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3 flex justify-between">
                ...
                {targetStudents.length > 0 && <span className="text-gray-900 bg-gray-100 px-2 py-0.5 rounded-md">{targetStudents.length} selected</span>}
              </p>
              <p className="text-xs text-gray-400 mb-3">If none selected, bills are generated for ALL students.</p>
            ...
            >
              {generating ? 'Generating...' : `Generate Bills ${targetStudents.length > 0 ? `(${targetStudents.length} students)` : '(All students)'}`}
            </button>
      ...
      {tab === 'structures' && (
        <div className="space-y-4">
          ...
          {/* Add form */}
          {showAddForm && (
            <form onSubmit={handleAddStructure} className="border border-gray-100 rounded-xl p-6 space-y-4">
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">New Fee Structure</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Name</label>
                  <input type="text" value={newName} onChange={e => setNewName(e.target.value)} required className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-gray-400 w-full" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Amount (₹)</label>
                  <input type="number" value={newAmount} onChange={e => setNewAmount(e.target.value)} required min={1} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-gray-400 w-full" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Fee Type</label>
                  <select value={newFeeType} onChange={e => setNewFeeType(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-gray-400 w-full">
                    <option value="combined">Hostel + Mess</option>
                    <option value="hostel">Hostel</option>
                    <option value="mess">Mess</option>
                ...
                <div>
                  ...
                  <select value={newBilling} onChange={e => setNewBilling(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-gray-400 w-full">
                    <option value="monthly">Monthly</option>
                    <option value="yearly">Yearly</option>
              ...
              <div className="flex gap-6">
                <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                  <input type="checkbox" checked={newIncludesHostel} onChange={e => setNewIncludesHostel(e.target.checked)} className="rounded" />
                  Includes Hostel
                </label>
                <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                  <input type="checkbox" checked={newIncludesMess} onChange={e => setNewIncludesMess(e.target.checked)} className="rounded" />
                  Includes Mess
                </label>
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">Description</label>
                <input type="text" value={newDesc} onChange={e => setNewDesc(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-gray-400 w-full" />
              </div>
              <button type="submit" disabled={loading} className="bg-gray-900 text-white rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-gray-700 transition-colors disabled:opacity-50">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {feeStructures.length === 0 ? (
                  <tr><td colSpan={5} className="text-center py-12 text-gray-400 text-sm">No fee structures yet</td></tr>
                ) : feeStructures.map(f => (
                  <tr key={f.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${f.is_active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
