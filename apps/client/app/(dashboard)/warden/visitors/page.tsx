...
export default function WardenVisitors() {
  ...
  const monthCount = visitors.filter(v => new Date(v.created_at).getMonth() === currentMonth).length;
      ...
      <div className="space-y-4">
        {filteredVisitors.length === 0 ? (
          ...
          filteredVisitors.map(v => (
            <div key={v.id} className="border border-gray-100 rounded-xl p-6 hover:border-gray-300 transition-colors">
              <div className="flex flex-col md:flex-row justify-between gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-gray-500">For: {v.students?.profiles?.full_name || 'Unknown Student'}</div>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="flex gap-2 mt-2">
                    {v.status === 'pending' && actionId !== v.id && (
                      <>
                        <button onClick={() => { setActionId(v.id); setActionType('approve'); }} className="text-xs px-3 py-1.5 rounded-lg border border-green-200 text-green-700 hover:bg-green-50 transition-colors">Approve</button>
                        <button onClick={() => { setActionId(v.id); setActionType('reject'); }} className="text-xs px-3 py-1.5 rounded-lg border border-red-200 text-red-700 hover:bg-red-50 transition-colors">Reject</button>
                      </>
                    )}
                    {v.status === 'approved' && (
                      <button onClick={() => handleAction(v.id, 'checkin')} className="text-xs px-4 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors">Check In</button>
                    )}
                    {v.status === 'checked_in' && (
                      <button onClick={() => handleAction(v.id, 'checkout')} className="text-xs px-4 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">Check Out</button>
                    )}
                  </div>
                  ...
                  {actionId === v.id && actionType && (
                    <div className="mt-2 w-full max-w-xs text-right space-y-2">
                      ...
                      <div className="flex justify-end gap-2">
                        <button onClick={() => { setActionId(null); setActionType(null); setNotes(''); }} className="text-xs text-gray-500 hover:text-gray-900">Cancel</button>
                        <button 
